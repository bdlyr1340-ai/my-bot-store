...
const BINANCE_TRANSFER_PENDING_POLL_INTERVAL_MS = Math.max(3 * 1000, parseInt(process.env.BINANCE_TRANSFER_PENDING_POLL_INTERVAL_MS || '5000', 10) || 5000);
const BINANCE_TRANSFER_IMMEDIATE_CHECK_DELAYS_MS = String(process.env.BINANCE_TRANSFER_IMMEDIATE_CHECK_DELAYS_MS || '5000,12000,20000').split(',').map(v => parseInt(String(v || '').trim(), 10)).filter(v => Number.isFinite(v) && v >= 1000 && v <= (30 * 60 * 1000));
...
const DEFAULT_TEXTS = {
  en: {
    ...
    mainDigitalSectionsButton: '📂 View',
    digitalSectionsGroupEmpty: 'No digital sections are available right now.',
  ...
  ar: {
    ...
    mainDigitalSectionsButton: '📂 عرض',
    digitalSectionsGroupEmpty: 'لا توجد خانات رقمية متاحة حالياً.',
...
Object.assign(DEFAULT_TEXTS.en, {
  ...
  fieldVerification: 'Authentication Link',
  fieldExtra: 'Extra',
  ...
  digitalStockInputPrompt: 'Send the stock/accounts now.\n\nQuick formats supported:\nemail|password\nemail | password\nemail|password|verification\nemail|password|verification|extra note\n\nUse the buttons below to add one account step by step or add Email+Pass+2FA directly.',
  ...
  switchToInviteMode: '🔄 Switch from stock to invitation',
  switchToStockMode: '🔄 Switch from invitation to stock',
  onDemandStock: 'On demand',
  activationEmailPrompt: 'Enter the email that should receive the activation invite:',
  activationRequestSent: '✅ Your activation request was received.\n\nService: {service}\nEmail: {email}\nAmount: {amount} USD\n\nThe admin will complete it soon.',
  ...
  activationAdminRejectWrongEmail: '📧 Email is wrong!',
  activationAdminComplete: '✅ Activated',
  ...
  activationCompletedUser: '✅ This email has been activated successfully.\nService: {service}\nEmail: {email}',
  activationCompletedAdmin: '✅ Activation request #{requestId} was marked as completed.',
  ...
  dailyGiftAdminCaption: 'Daily gift'
});
...
Object.assign(DEFAULT_TEXTS.ar, {
  ...
  activationEmailPrompt: 'أرسل الإيميل الذي سيتم إرسال دعوة التفعيل إليه:',
  activationRequestSent: '✅ تم استلام طلب التفعيل.\n\nالخدمة: {service}\nالإيميل: {email}\nالمبلغ: {amount} دولار\n\nسيقوم الأدمن بإكماله قريباً.',
  activationAdminNotice: '📩 طلب تفعيل جديد\n\nالخدمة: {service}\nالاسم: {name}\nالمعرف: {username}\nايدي المستخدم: {userId}\nالإيميل: {email}\nالمبلغ: {amount} دولار',
  activationAdminSourceStock: '📦 المصدر: مخزون',
  activationAdminSourceInvite: '🎟 المصدر: دعوة',
  activationAdminRejectWrongEmail: '📧 الايميل خطأ!',
  activationAdminComplete: '✅ تم التفعيل',
  ...
  activationCompletedUser: '✅ تم تفعيل هذا الإيميل بنجاح.\nالخدمة: {service}\nالإيميل: {email}',
  activationCompletedAdmin: '✅ تم وضع طلب التفعيل رقم #{requestId} كمكتمل.',
  ...
  dailyGiftAdminCaption: 'هدية يومية',
  ...
  fieldVerification: 'رابط المصادقة',
  ...
  digitalStockInputPrompt: 'أرسل الآن المخزون/الحسابات.\n\nالصيغ السريعة المدعومة:\nemail|password\nemail | password\nemail|password|verification\nemail|password|verification|ملاحظة إضافية\n\nاستخدم الأزرار بالأسفل لإضافة حساب خطوة بخطوة أو لإضافة إيميل+باس+رابط مصادقة مباشرة.',
...
async function getActivationAdminReplyMarkup(requestId) {
  return {
    inline_keyboard: [
      [{ text: await getText(ADMIN_ID, 'activationAdminComplete'), callback_data: `admin_activation_complete_${requestId}` }],
...
function getMerchantMetaConfig(merchant) {
  if (!merchant?.description || typeof merchant.description !== 'object') return {};
  return merchant.description.meta && typeof merchant.description.meta === 'object'
    ? { ...merchant.description.meta }
    : {};
}

function setMerchantMetaConfig(merchant, patch = {}) {
  const description = merchant?.description && typeof merchant.description === 'object'
...
async function isEmailActivationProduct(merchant) {
  ...
  return Boolean(meta.inviteMode || meta.requiresEmailActivation);
}

function formatTimeLeft(ms, lang = 'en') {
...
async function getDailyGiftAmount() {
  const rawValue = await getGlobalSetting('daily_gift_amount', '0.05');
  const value = parseFloat(rawValue);
...
async function claimDailyGift(userId) {
  const amount = await getDailyGiftAmount();
  const key = `daily_gift_last_${userId}`;
  const rawLast = await getGlobalSetting(key, '');
  const now = new Date();
  if (rawLast) {
    ...
    if (!Number.isNaN(last.getTime())) {
      ...
      if (diff < cooldown) {
        const user = await User.findByPk(userId);
        return { success: false, remainingMs: cooldown - diff, lang: user?.lang || 'en' };
      }
    }
  }
  await User.increment({ balance: amount }, { where: { id: userId } });
  await BalanceTransaction.create({ userId, amount, type: 'daily_gift', status: 'completed', caption: 'Daily gift' });
  await Setting.upsert({ key, lang: 'global', value: now.toISOString() });
  const user = await User.findByPk(userId);
  return { success: true, amount, balance: Number(user?.balance || 0), lang: user?.lang || 'en' };
}
...
async function getMenuButtonItems(userId) {
  return [
    ...
    { id: 'daily_gift', name: await getText(userId, 'dailyGift') },
    ...
    { id: 'digital_sections_group', name: await getText(userId, 'mainDigitalSectionsButton') },
...
async function showDepositOptionsAdmin(userId) {
  ...
  const keyboard = {
    inline_keyboard: [
      [{ text: `${visibility.IQD !== false ? '✅' : '❌'} ${await getText(userId, 'depositOptionIQD')}`, callback_data: 'admin_toggle_deposit_option_IQD' }],
      [{ text: `${visibility.USD !== false ? '✅' : '❌'} ${await getText(userId, 'depositOptionUSD')}`, callback_data: 'admin_toggle_deposit_option_USD' }],
      [{ text: `${visibility.BINANCE_AUTO !== false ? '✅' : '❌'} ${await getText(userId, 'depositOptionBinanceAuto')}`, callback_data: 'admin_toggle_deposit_option_BINANCE_AUTO' }],
      [{ text: await getText(userId, 'back'), callback_data: 'admin_manage_deposit_settings' }]
    ]
  };
...
// -------------------------------------------------------------------
// تعديل showCurrencyOptions لإظهار الشحن اليدوي والبايننس الأوتوماتيكي
async function showCurrencyOptions(userId) {
  ...
  if (visibility.IQD !== false) {
    rows.push([{ text: `💵 ${await getDepositDisplayName(userId, 'IQD')}`, callback_data: 'deposit_currency_iqd' }]);
  }

  if (visibility.USD !== false) {
    rows.push([{ text: `💳 ${await getDepositDisplayName(userId, 'USD')}`, callback_data: 'deposit_currency_usd' }]);
  }

  if (visibility.BINANCE_AUTO !== false) {
    rows.push([{ text: '⚡ Binance (USDT)', callback_data: 'deposit_binance_auto' }]);
  }

  rows.push([{ text: await getText(userId, 'back'), callback_data: 'back_to_menu' }]);

  const availableLines = [];
  if (visibility.IQD !== false) availableLines.push(`• ${await getDepositDisplayName(userId, 'IQD')}`);
  if (visibility.USD !== false) availableLines.push(`• ${await getDepositDisplayName(userId, 'USD')}`);
  if (visibility.BINANCE_AUTO !== false) availableLines.push('• ⚡ Binance (USDT)');

  const extraText = availableLines.length
    ? `

${lang === 'ar' ? 'طرق الشحن المتاحة حالياً:' : 'Available topup methods:'}
${availableLines.join('\n')}`
    : '';
...
function scheduleBinanceTransferImmediateChecks(userId) {
  for (const delayMs of BINANCE_TRANSFER_IMMEDIATE_CHECK_DELAYS_MS) {
    const timer = setTimeout(async () => {
      try {
        const pendingUser = await User.findByPk(userId, { attributes: ['state'] });
        const pendingState = safeParseState(pendingUser?.state);
        if (pendingState?.action !== 'binance_transfer_pending') return;
        await processBinanceAutoVerification(userId, pendingState, { rawInput: '', silent: true });
      } catch (err) {
        console.error(`Binance transfer immediate check error for user ${userId}:`, err.message);
      }
    }, delayMs);
...
async function sendBinanceAutoInstructions(userId, amount) {
  ...
  scheduleBinanceTransferImmediateChecks(userId);
}
...
async function sendMainMenu(userId) {
  ...
  const buttonLabels = {
    ...
    daily_gift: await getText(userId, 'dailyGift'),
    ...
    digital_sections_group: await getText(userId, 'mainDigitalSectionsButton'),
  ...
  for (const product of mainScreenProducts) {
    ...
    buttons.push([{
      text: await getText(userId, 'mainScreenDigitalProductButton', { name, price: formatUsdPrice(product.price), stock: stockLabel }),
...
المتبقي في المخزون: ${result.remaining}`);
    ...
    if (data === 'digital_sections_group') {
      await showDigitalSectionsGroupForUser(userId);
      await cleanupPressedMessage();
      await bot.answerCallbackQuery(query.id);
      return;
    }
    ...
    const adminActivationCompleteMatch = data.match(/^admin_activation_complete_(\d+)$/);
    if (adminActivationCompleteMatch && isAdmin(userId)) {
      const request = await ActivationRequest.findByPk(parseInt(adminActivationCompleteMatch[1], 10));
      ...
      request.delayUntil = null;
      await request.save();
      await bot.sendMessage(request.userId, await getText(request.userId, 'activationCompletedUser', {
        service: await getActivationRequestServiceName(request, request.userId),
        email: request.email
      })).catch(() => null);
      await bot.answerCallbackQuery(query.id, { text: await getText(userId, 'activationCompletedAdmin', { requestId: request.id }) });
      return;
    }
...
${await getText(userId, 'activationDelayNotifyUserPrompt')}`, {
    ...
    if (data === 'daily_gift') {
      ...
      if (result.success) {
        await bot.sendMessage(userId, await getText(userId, 'dailyGiftClaimed', {
          amount: formatUsdPrice(result.amount),
          balance: formatUsdPrice(result.balance)
        }));
        await bot.sendMessage(ADMIN_ID, `🎁 ${await getText(userId, 'dailyGiftAdminCaption')}
User ID: ${userId}
Amount: ${formatUsdPrice(result.amount)} USD`).catch(() => {});
      } else {
        await bot.sendMessage(userId, await getText(userId, 'dailyGiftAlreadyClaimed', {
          timeLeft: formatTimeLeft(result.remainingMs, result.lang)
        }));
      }
      await cleanupPressedMessage();
      await bot.answerCallbackQuery(query.id);
    ...
    if (data.startsWith('digital_buy_')) {
      ...
      if (await isEmailActivationProduct(merchant)) {
        await setUserState(userId, { action: 'digital_email_activation_purchase', merchantId, discountCode });
        await bot.sendMessage(userId, await getText(userId, 'activationEmailPrompt'), {
...
bot.on('message', async msg => {
  ...
  try {
    ...
    const trimmedIncomingText = typeof text === 'string' ? String(text).trim() : '';
    if (/^\/start(?:\s|$)/i.test(trimmedIncomingText)) {
      return;
    }
...
${await getText(userId, 'transactionIdPlainLine', { id: transactionId })}`);
    ...
    if (state?.action === 'deposit_amount') {
      ...
      const currency = String(state.currency || 'USD').toUpperCase() === 'IQD' ? 'IQD' : 'USD';
      await showPaymentMethodsForDeposit(userId, amount, currency);
      return;
    }

    if (state?.action === 'deposit_awaiting_proof') {
      const rawMessage = String(text || msg.caption || '').trim();
      const imageFileId = photo ? photo[photo.length - 1].file_id : null;
      const attachmentId = imageFileId || null;

      if (!rawMessage && !attachmentId) {
        await bot.sendMessage(userId, await getText(userId, 'depositProofReceived'));
        return;
      }

      await requestDeposit(userId, Number(state.amount || 0), String(state.currency || 'USD').toUpperCase(), rawMessage, attachmentId, msg.from || null);
      await clearUserState(userId);
      await bot.sendMessage(userId, await getText(userId, 'depositProofReceived'));
      await sendMainMenu(userId);
      return;
    ...
    if (state?.action === 'binance_auto_session' || state?.action === 'binance_auto_waiting_proof') {
      await clearUserState(userId);
      await bot.sendMessage(userId, await getText(userId, 'binancePayStatusExpired'));
      await showBinanceAutoAmountOptions(userId);
      return;
...
}, BINANCE_TRANSFER_PENDING_POLL_INTERVAL_MS);
